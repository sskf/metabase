(ns metabase-enterprise.semantic-layer.cli
  "CLI entrypoint for computing the semantic-layer complexity score against a representation
  directory (see [[metabase-enterprise.semantic-layer.representation]]). Intended for offline
  scoring in benchmarks, CI, or exploratory analysis where a live Metabase instance isn't
  available.

  Invoke via:

    clojure -M:ee:dev -m metabase-enterprise.semantic-layer.cli \\
  ...
  (:require
   ...
   [metabase-enterprise.semantic-layer.representation :as representation]))
...
(defn- usage [summary]
  (str "Usage: clojure -M:ee:dev -m metabase-enterprise.semantic-layer.cli [options]\n\n"
       "Options:\n" summary))
