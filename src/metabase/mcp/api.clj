...
(defn- handle-pending-card-store
  "Handle PATCH /api/mcp — store a base64-encoded query for the upcoming render_drill_through
...
(def ^{:arglists '([request respond raise])} handler
  ...
  (open-api/handler-with-open-api-spec
   (fn [request respond raise]
     (let [;; PATCH is our custom pending-card endpoint, not part of MCP protocol.
           ...
           origin-error    (when (not= :patch (:request-method request))
                     ...
                     (try
                       (let [request (assoc request :token-scopes token-scopes)]
                         (case (:request-method request)
                           ...
                           :patch  (respond (handle-pending-card-store user-id request))
